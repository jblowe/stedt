...
The included sql file (STEDT_public_20160602.sql.bz2) contains a dump of the STEDT database.  MySQL dumps are a sequence of MySQL commands for recreating the STEDT database in a database called 'stedt'. It's a large file that may choke graphical MySQL clients, so executing it on the command line with the 'mysql' utility is recommended.

Instruction on how to "restore" the database to a MySQL server of your choice may be found in the nearby file README_restore.

All relevant tables (and some irrelevant ones) have been included.

The database is most easily navigated and manipulated using the web interface provided in the 'rootcanal' project code. This is also
described in README_restore.
...
The dump was created using the following script, which "sanitizes" the database of user data and clears other log data.

   https://github.com/stedt-project/sss/blob/master/archiving/dump/dump_public.pl

Depending on the system you are using, it may take a few minutes to run.

$ ./dump_public.pl

6 inactive users deleted.
23 email addresses cleared.
22 passwords reset to 'pass'.
0E0 sessions deleted.
0E0 queries deleted from querylog.
Beginning database dump...
Bzipping it...
Completed!
