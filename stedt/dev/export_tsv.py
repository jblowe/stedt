...
def safe(name):
    return re.sub(r"[^A-Za-z0-9._-]", "_", name) or "_blank"
...
def main():
    ...
    for r in c.execute("SELECT spec,rn,tag,id,notetype,xmlnote,ord,noteid FROM notes ORDER BY ord, noteid"):
        ...
        if r["spec"] == "E":
            ...
            snotes.append((clean(r["id"]), t, r["xmlnote"]))
        elif r["spec"] == "L":
            lnotes.append((r["rn"], t, r["xmlnote"]))
    ...
    for r in c.execute("SELECT * FROM mesoroots ORDER BY tag, id"):
        meso_rows.append(
            [
                ...
                clean(r["gloss"]),
                clean(r["variant"]),
                (clean(r["old_tag"]) if r["old_tag"] else ""),
                clean(r["old_note"]),
            ]
        )
    ...
    # ================= reference tables =================
    write_tsv(
        f"{DEST}/thesaurus.tsv",
        ["semkey", "title", "semcat", "old_chapter", "old_subchapter"],
        [
            [
                clean(r["semkey"]),
                clean(r["chaptertitle"]),
                clean(r["semcat"]),
                clean(r["old_chapter"]),
                clean(r["old_subchapter"]),
            ]
            for r in c.execute("SELECT * FROM chapters ORDER BY id")
            if r["semkey"]
        ],
    )
    write_tsv(f"{DEST}/chapter_notes.tsv", ["semkey", "type", "text"], [[clean(k), t, x] for k, t, x in cnotes])

    write_tsv(
        f"{DEST}/languages.tsv",
        [
            "lgid",
            "name",
            "abbr",
        ...
        [
            [
                ...
                clean(r["picode"]),
                clean(r["pinotes"]),
                (clean(r["pi_page"]) if r["pi_page"] else ""),
                clean(r["notes"]),
            ]
    ...
    write_tsv(
        f"{DEST}/languagegroups.tsv",
        ["grpid", "grpno", "abbr", "name", "proto_language", "genetic", "grp0", "grp1", "grp2", "grp3", "grp4"],
        [
            [
                clean(r["grpid"]),
                clean(r["grpno"]),
    ...
    write_tsv(
        f"{DEST}/glosswords.tsv",
        ["word", "rn", "semcat", "subcat", "semkey"],
        [
    ...
    for rn, grp_rows in groupby(hashrows, key=lambda r: r["rn"]):
        slots = ["|".join(clean(x["tag_str"]) for x in srows) for _, srows in groupby(grp_rows, key=lambda r: r["ind"])]
    ...
    for r in c.execute("SELECT * FROM lexicon"):
        ...
        wl.setdefault(src, []).append(
            [
                clean(r["rn"]),
                clean(r["lgid"]),
                (ln["language"] if ln else ""),
                clean(r["reflex"]),
                clean(r["originalreflex"]),
                clean(r["gloss"]),
                clean(r["originalgloss"]),
                clean(r["gfn"]),
                clean(r["originalgfn"]),
                clean(r["semkey"]),
                clean(r["srcid"]),
    ...
    for rn, t, x in lnotes:
        if rn in rn_src:
            src_lnotes.setdefault(rn_src[rn], []).append([clean(rn), t, x])
        else:
            orphan_lnotes.append([clean(rn), t, x])
    ...
    for s in sorted(all_srcs):
        ...
        if s in src_rows:
            write_tsv(f"{d}/source.tsv", SRC_COLS, [src_rows[s]])
        if s in wl:
        ...
        if s in src_anno:
            write_tsv(f"{d}/annotations.tsv", ["type", "text"], src_anno[s])
    ...
    orphan_links = [
        [clean(r["rn"]), clean(r["ind"]), clean(r["tag_str"])]
    ...
    print(
        f"  sources {len(all_srcs)} ({sum(1 for s in all_srcs if s in wl)} with wordlists)  "
        f"reflexes {sum(len(v) for v in wl.values())}"
    )
    print(
        f"  reflex_notes {sum(len(v) for v in src_lnotes.values())} (+{len(orphan_lnotes)} orphan)  "
