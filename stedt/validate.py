...
One check_* function per section, run in order by main(); err()/warn() collect into the
module-level lists the final report drains.
"""

import csv, glob, os, re, sys

from stedt.paths import DATA as ROOT, DB, ROOT as REPO
...
def check_reference():
    """The reference tables: groups, thesaurus keys, languages — keys + field shapes."""
    ...
    lgids, lgname = set(), {}
    for l in read_tsv(f"{ROOT}/languages.tsv", COLS["languages"]):
    ...
    return grpids, semkeys, lgids, lgname


def check_etyma(grpids, semkeys):
    """etyma.tsv: keys, statuses, filing keys, sequence/public shapes."""
    etyma_tags = set()
...
def check_etyma_children(etyma_tags, grpids):
    """mesoroots + etymon notes must reference an existing etymon (and subgroup anchor)."""
...
def _analysis_toks(analysis):
    for slot in (analysis or "").split(","):
        yield from slot.split("|")


def check_sources(lgids, lgname, semkeys, etyma_tags):
    """Per-source folders: source.tsv + wordlist.tsv + notes.tsv + annotations.tsv.
    Returns (seen_rn {rn: folder}, srcabbrs)."""
    ...
    for srcdir in sorted(glob.glob(f"{ROOT}/sources/*")):
        ...
        for row in (read_tsv(wlp, COLS["wordlist"]) if os.path.exists(wlp) else []):
            ...
            for tok in _analysis_toks(row["analysis"]):
                m = re.match(r"\d+", tok)
                if m:
    ...
    return seen_rn, srcabbrs


def check_glosswords(seen_rn):
...
def check_orphans(seen_rn, etyma_tags):
    """The orphan tables: rns outside any wordlist are expected, shapes still checked."""
...
def check_sync_markers():
    ...
    A `SYNC(<name>)` comment marks code whose output must stay identical to a counterpart in
    another file (Python↔JS, main↔legacy, Python↔CSS) — the codebase's most repeated bug class
    is fixing one side and missing the other. A name found in only ONE file means a twin lost
    its marker (or the marked twin was deleted): fail loudly. Scans the working tree (paths.ROOT),
    never site/."""
    scan = (
        glob.glob(os.path.join(REPO, "stedt", "**", "*.py"), recursive=True)
        + glob.glob(os.path.join(REPO, "web", "src", "*.js"))
        + glob.glob(os.path.join(REPO, "static", "*.css"))
        + glob.glob(os.path.join(REPO, "stedt", "render", "templates", "*.html"))
    )
    sync = {}
    for p in scan:
        if "__pycache__" in p:
            continue
        for name in set(re.findall(r"SYNC\(([\w-]+)\)", open(p, encoding="utf-8").read())):
            sync.setdefault(name, set()).add(os.path.relpath(p, REPO))
    for name, files in sorted(sync.items()):
        if len(files) < 2:
            err(f"SYNC({name}) marked in only one file ({next(iter(files))}) — its twin is unmarked or gone")
    print(f"  sync markers: {len(sync)} twins across {len(set().union(*sync.values()) if sync else set())} files")


def check_inline_styles():
    """Templates carry no static inline styles.
    A literal style="…" in a template is unoverridable from site.css (it kept blocking
    restyling); style constants belong in classes. Values interpolating template data ({{ … }})
    are exempt — those are computed indents, parameterized rather than constant."""
    for p in glob.glob(os.path.join(REPO, "stedt", "render", "templates", "*.html")):
        for m in re.finditer(r'style="([^"]*)"', open(p, encoding="utf-8").read()):
            if "{{" not in m.group(1):
                err(f"{os.path.relpath(p, REPO)}: static inline style=\"{m.group(1)[:50]}\" — move it to a class in site.css")


def check_css():
    """site.css holds the floor: no ghost rules, and every max-width is allowlisted.

    Ghost rules: every class selector must be referenced somewhere in the markup sources
    (templates, the renderer, the client JS) — orphaned rules sat unwired for a whole release
    once (.citebox) and the torn-out edit flow left a family of corpses. The scan is
    conservative: a class whose name happens to occur as a plain word anywhere in the sources
    passes, so it only catches the unambiguous ghosts; that still holds the floor at zero.

    max-width: em-measure caps on text blocks kept regressing pages to ~60% width, found one
    symptom at a time (search syntax reference, Chinese comparanda, subgroup notes — review
    findings 2026-06-10/11). Standing design rule: running text fills the main column. A
    max-width in site.css must appear on the allowlist — adding one is a deliberate design
    decision, made visibly here, not a habit."""
    css = re.sub(r"/\*.*?\*/", "", open(os.path.join(REPO, "static", "site.css"), encoding="utf-8").read(), flags=re.S)
    classes = set()
    ...
    corpus = "".join(
        ...
        for pat in ("stedt/render/templates/*.html", "stedt/render/*.py", "web/src/*.js", "static/*.js")
        for p in glob.glob(os.path.join(REPO, *pat.split("/")))
    )
    for c in sorted(classes):
        if not re.search(r"\b" + re.escape(c) + r"\b", corpus):
            err(f"site.css: class .{c} is referenced nowhere in templates/renderer/client JS — dead rule")
    print(f"  css: {len(classes)} classes, all referenced")
    ...
    for sel, body in re.findall(r"([^{}@]+)\{([^{}]*)\}", css):
        for v in re.findall(r"max-width\s*:\s*([^;}]+)", body):
            key = (sel.strip().split(",")[0].strip(), v.strip())
            if key not in MAXW_OK:
                err(f"site.css: max-width {key[1]!r} on {key[0]!r} — running text fills the main column; "
                    "if this cap is deliberate, allowlist it in validate.py")
    print(f"  css: max-width allowlist holds ({len(MAXW_OK)} approved caps)")


def check_render_invariants():
    ...
    both workflows); without it the section is skipped as a warning so a bare data/-only
    checkout can still validate."""
    if os.path.exists(DB):
        from stedt.dev import invariants
...
def main():
    grpids, semkeys, lgids, lgname = check_reference()
    etyma_tags = check_etyma(grpids, semkeys)
    check_etyma_children(etyma_tags, grpids)
    seen_rn, srcabbrs = check_sources(lgids, lgname, semkeys, etyma_tags)
    check_glosswords(seen_rn)
    check_hptb(etyma_tags)
    check_orphans(seen_rn, etyma_tags)
    print(
