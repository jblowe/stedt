...
def s(v):
    return None if v in (None, "") else v  # '' -> NULL (absent optional field)
...
def main():
    ...
    def insert(t, rowdicts):
        cols = TABLES[t]
        c.executemany(
            f"INSERT INTO {t} VALUES ({','.join('?'*len(cols))})", [[d.get(k) for k in cols] for d in rowdicts]
        )
    ...
    insert(
        ...
        [
            ...
            for g in rows(f"{ROOT}/languagegroups.tsv")
        ],
    )
    insert(
        "languagenames",
        [
            {
                ...
                "lgid": i(l["lgid"]),
                "grpid": i(l["grpid"]),
                "srcofdata": s(l["srcofdata"]),
                "picode": s(l["picode"]),
                "pinotes": s(l["pinotes"]),
                "pi_page": i(l["pi_page"]),
            }
            for l in rows(f"{ROOT}/languages.tsv")
        ],
    )
    ...
    for n, t in enumerate(rows(f"{ROOT}/thesaurus.tsv"), 1):
        chap.append(
            {
                "semkey": t["semkey"],
                "chaptertitle": s(t["title"]),
                "semcat": s(t["semcat"]),
                "old_chapter": s(t["old_chapter"]),
                "old_subchapter": s(t["old_subchapter"]),
                "id": n,
            }
        )
    insert("chapters", chap)
    for cn in rows(f"{ROOT}/chapter_notes.tsv"):
        add_note("C", cn["semkey"], cn["type"], cn["text"])
        ...
        hptb.append(
            {
                "hptbid": i(h["hptbid"]),
                "plg": s(h["plg"]),
                "protoform": s(h["protoform"]),
                "protogloss": s(h["gloss"]),
                "pages": s(h["pages"]),
                "mainpage": s(h["mainpage"]),
                "init": s(h["init"]),
                "bare": s(h["allofams"]),
                "semclass1": s(h["semclass"]),
                "semclass2": s(h["semclass2"]),
                "tags": s(h["tags"]),
            }
        )
    ...
    def with_id(rs):
        return [{**r, "id": n} for n, r in enumerate(rs, 1)]

    insert(
        "otherchapters",
        with_id(
            [
                {k: s(r[k]) for k in ("chapter", "heading", "semcat", "subcat", "cf", "n")}
    ...
    insert(
        ...
        with_id(
            [
                {k: s(r[k]) for k in ("chapter", "subchapter", "semcat", "heading", "frqdb", "frqsubcats")}
    ...
    for d in rows(f"{ROOT}/etyma.tsv"):
        ety.append(
            {
                "chapter": d["chapter"] or d["semkey"] or "",
                "sequence": float(d["sequence"]) if d["sequence"] else None,
                "tag": i(d["tag"]),
                "grpid": i(d["grpid"]),
                "protoform": s(d["protoform"]),
                "protogloss": s(d["gloss"]),
                "xrefs": s(d["xrefs"]),
                "notes": s(d["references"]),
                "possallo": s(d["possallo"]),
                "allofams": s(d["allofams"]),
                "status": d["status"],
                "public": 1 if d["public"] == "1" else 0,
                "exemplary": "x" if d["exemplary"] == "x" else "",
                "semkey": d["semkey"] or "",
    ...
    for srcdir in sorted(glob.glob(f"{ROOT}/sources/*")):
        for b in rows(f"{srcdir}/source.tsv"):
            srcbib.append({k: (i(b.get(k)) if k in ("scope", "infascicle") else s(b.get(k))) for k in TABLES["srcbib"]})
        for b in rows(f"{srcdir}/source.tsv"):
            for a in rows(f"{srcdir}/annotations.tsv"):
                add_note("S", b["srcabbr"], a["type"], a["text"])
            ...
            lex.append(
                {
                    "status": row["status"],
                    "reflex": row["reflex"],
                    "originalreflex": row["originalreflex"],
                    "gloss": row["gloss"],
                    "originalgloss": row["originalgloss"],
                    "gfn": row["gfn"],
                    "originalgfn": row["originalgfn"],
                    ...
                    "lgid": i(row["lgid"]),
                    "semkey": row["semkey"],
                    "src_set_rn": i(row["src_set_rn"]) or 0,
                }
            )
    ...
    insert("srcbib", srcbib)
    insert("lexicon", lex)
    insert("lx_et_hash", links)
    ...
    for stmt in [
        ...
        "CREATE INDEX ix_ln_src ON languagenames(srcabbr)",
